...
import { getFirstRequiredString, getOptionalImageFile, getRequiredString } from "@/lib/form";
...
export async function POST(request: Request) {
  ...
  const detail = getFirstRequiredString(formData, ["detail", "jadwal", "namaLatin"]);
  if (!nama || !detail) {
    return errorResponse("VALIDATION_ERROR", "Field nama dan detail wajib diisi.", 422);
  }
  ...
  try {
    ...
    console.error("POST /obat failed", error);
